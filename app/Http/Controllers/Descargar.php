...
class Descargar extends Controller
{
    public function enviar(){
        return view('descargarpdf');
    }


  public function buscarCaracter($uuid){
        $carac = array(',','.',';',':','|','/');
        for($i=0;$i<count($carac);$i++){
            if(strpos($uuid,$carac[$i])){
                return $carac[$i];
            }
        }
    ...
    public function obtieneReciboPDFHist(Request $request){///colores rosa y verde
        ...
        if($request->uuid == ''){
            ...
            $folios = explode($this->buscarCaracter($request->uuid),$request->uuid);
            //dd($folios);
        ...
        if($switchColor > $tiempoCambio){
            $pdf = \PDF::loadHTML(\View::make('recibosV.'.strtolower($sanitizedData['data']['tipoAddenda']))->with($sanitizedData))
                    ->setPaper([0,0,612.00,792.00], 'portrait')
                    ->setWarnings(false)
                    ->save(storage_path('pdf/' . $request->uuid.'_'.$fechaGeneracion->format('Y-m-d')) . '.pdf' );
                    //->download($request->uuid . '.pdf');
                    if($pdf == TRUE && $i==count($folios)-1){
                        return view('descargarpdf');
                    }
        }else{
            $pdf = \PDF::loadHTML(\View::make('recibos.'.strtolower($sanitizedData['data']['tipoAddenda']))->with($sanitizedData))
            ->setPaper([0,0,612.00,792.00], 'portrait')
            ->setWarnings(false)
            ->save(storage_path('pdf/' . $request->uuid.'_'.$fechaGeneracion->format('Y-m-d')) . '.pdf' );
            //->download($request->uuid . '.pdf');
            ...
            //dd('no existe el raaafc');//crear vista de error
            return \View::make('descargarpdf')->with(['message'=>['class'=>'alert-danger','normal'=>$respuesta['error']['msg']]]);
        }
        }
         }
    }
    ...
    private function generarReciboHist(Request $request, $servicio){ //mulu
        if (isset($request->collection)) {
            return $respuesta = $this->generarRecibo($request,'/recibo/obtenerReciboElectronicoGetUUID');
        }
    ...
    private function sanatizeInfoHist1003($data){/// tipo recibo pbi
       ...
       //  dd($data['cfdi:Comprobante']['cfdi:Addenda']['Addenda']['SalarioDiarioIntegrado']);
         $retorno['UNIDADADMINISTRATIVA'] = (isset($data['cfdi:Comprobante']['cfdi:Addenda']['Addenda']['UNIDADADMINISTRATIVA']))?$data['cfdi:Comprobante']['cfdi:Addenda']['Addenda']['UNIDADADMINISTRATIVA']:""; //si
            ...
            /* if(isset($data['cfdi:Comprobante']['cfdi:Addenda']['AddendaPercepciones']['AddendaPercepcion']))
                 $varArray1 = $data['cfdi:Comprobante']['cfdi:Addenda']['AddendaPercepciones']['AddendaPercepcion'];
             if(isset($varArray1))
                 $data['cfdi:Comprobante']['cfdi:Addenda']['AddendaPercepciones']['AddendaPercepcion'] = array( 0 => $varArray1);
             foreach($data['cfdi:Comprobante']['cfdi:Addenda']['AddendaPercepciones']['AddendaPercepcion'] as $percepciones)
             ...
             {
                 // $percepciones[0] = array( 0 => $percepciones[0]);
                 //dd($percepciones);
                 foreach($percepciones as $detallePercepcion){
                     //dd($detallePercepcion['Clave']);
                     if($detallePercepcion['Clave'] != '0000'){
                     
                         $bandera++;
                         $regresaConceptoPercepcion .= $detallePercepcion['Clave'].'<br>';
                         $regresaDescripcionPercepcion .= $detallePercepcion['Concepto'].'<br>';
                         $regresaImportePercepcion .= number_format($detallePercepcion['ImporteGravado'], 2,'.',',').'<br>';
         ...
         //$retorno['bandera'] = $bandera;
           ...
           return $retorno;
            ...
            /* if(isset($data['cfdi:Comprobante']['cfdi:Addenda']['AddendaDeducciones']['AddendaDeduccion']))
             $varArray2 = $data['cfdi:Comprobante']['cfdi:Addenda']['AddendaDeducciones']['AddendaDeduccion'];
 
             if(isset($varArray2))
             $data['cfdi:Comprobante']['cfdi:Addenda']['AddendaDeducciones']['AddendaDeduccion'] = array( 0 => $varArray2); //Deduccion
 
             if(!empty($data['cfdi:Comprobante']['cfdi:Addenda']['AddendaDeducciones']['AddendaDeduccion']) && isset($data['cfdi:Comprobante']['cfdi:Addenda']['AddendaDeducciones']['AddendaDeduccion'])){
 ...
 
             {
                             ...
                             foreach($deducciones as $detalleDeduccion){
                     
                     
                                 if($detalleDeduccion['Clave'] != '0000'){
                                 
                                     $bandera++;
                                     
                                     $regresaDescripcionDeduccion .= $detalleDeduccion['Concepto'].'<br>';
                                     $regresaImporteDeduccion .= number_format($detalleDeduccion['ImporteTotal'], 2,'.',',').'<br>';
